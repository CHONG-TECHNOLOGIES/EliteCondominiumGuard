...
import { Dumbbell, Plus, Edit2, Trash2, Loader2, Search, X, Building2, ChevronDown, Check } from 'lucide-react';
...
export default function AdminSports() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [sportsData, condosData] = await Promise.all([
        api.adminGetAllSports(filterCondoId || undefined),
  ...
  const handleCreate = async () => {
    ...
    try {
      const result = await api.adminCreateSport(formData);
      if (result) {
        await loadData();
        setShowCreateModal(false);
        resetForm();
        showToast('success', 'Desporto criado com sucesso!');
      } else {
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      if (result) {
        ...
        resetForm();
        showToast('success', 'Desporto atualizado com sucesso!');
      } else {
  ...
  const handleDelete = async (sport: Sport) => {
    showConfirm(
      `Deseja realmente remover o desporto ${sport.name}?`,
      async () => {
        try {
          const result = await api.adminDeleteSport(String(sport.id));
          if (result) {
            await loadData();
            showToast('success', 'Desporto removido com sucesso!');
          } else {
